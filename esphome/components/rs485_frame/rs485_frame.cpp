...
void RS485FrameHub::loop() {
  ...
  this->maybe_tx_(now);
  // Sample micros() once and use it for both the readiness check and the lateness recording
  // so the two values are coherent (a second call between them adds spurious latency).
  // Unsigned subtraction wraps correctly so this comparison handles the 49-day rollover.
  {
    uint32_t check_us = micros();
    if (this->tx_start_pending_ && check_us - this->tx_start_at_us_ < 0x80000000UL) {
      this->tx_start_pending_ = false;
#ifdef USE_RS485_FRAME_SNIFFER_STATS
      if (this->sniffer_stats_ != nullptr)
        this->sniffer_stats_->record_tx_lateness(check_us - this->tx_start_at_us_);
#endif
...
void RS485FrameHub::read_uart_(uint32_t now) {
  ...
  while (this->available() && this->read_byte(&byte)) {
#ifdef USE_RS485_FRAME_SNIFFER_STATS
    if (this->sniffer_stats_ != nullptr && this->loop_rx_bytes_ < UINT32_MAX)
      this->loop_rx_bytes_++;
#endif
